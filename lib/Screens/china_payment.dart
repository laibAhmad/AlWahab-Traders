...
class ChinaPayment extends StatefulWidget {
  const ChinaPayment({Key? key}) : super(key: key);

  @override
  State<ChinaPayment> createState() => _ChinaPaymentState();
}
...
class _ChinaPaymentState extends State<ChinaPayment> {
  ...
  Future<List<Expenses>> getData(int n) async {
    ...
    if (n == 0) {
      await paymentRef.get().asStream().forEach((element) {
    ...
    return Column(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(color: Colors.brown.withOpacity(0.15)),
